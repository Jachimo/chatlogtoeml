...
- Implemented `chatlogtoeml/parsers/apple_db.py` (SQLite parser). Key features:
  ...
  - Message BLOB body decoding for `attributedBody` and `payload_data`:
    - typedstream decoding via `pytypedstream`
...
Notes / next steps:
- Typedstream and NSKeyedArchiver decoders are now treated as hard dependencies for MVP.
